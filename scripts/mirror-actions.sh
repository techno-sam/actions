...
TAG=$1
GITHUB_LOGIN="techno-sam"
RELEASE_BODY="Details available at [techno-sam/actions@${TAG}](https://github.com/techno-sam/actions/releases/tag/${TAG})."
...
if [[ "$GITHUB_REPOSITORY" != "techno-sam/actions" ]]; then
  echo "This mirror script is only meant to be run from techno-sam/actions, not ${GITHUB_REPOSITORY}. Nothing to do here."
  exit 0
fi
...
git clone "https://techno-sam:${GITHUB_SUPER_TOKEN}@github.com/techno-sam/${ACTION_KEYBASE_NOTIFICATIONS_REPO}.git" /tmp/${ACTION_KEYBASE_NOTIFICATIONS_REPO}
...
git clone "https://techno-sam:${GITHUB_SUPER_TOKEN}@github.com/techno-sam/${ACTION_AUTOMATIC_RELEASES_REPO}.git" /tmp/${ACTION_AUTOMATIC_RELEASES_REPO}
...
git clone "https://techno-sam:${GITHUB_SUPER_TOKEN}@github.com/techno-sam/${ACTION_INJECT_SSM_SECRETS_REPO}.git" /tmp/${ACTION_INJECT_SSM_SECRETS_REPO}
